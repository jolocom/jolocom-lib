...
export class EcdsaLinkedDataSignature
  ...
  /**
   * Get the hex encoded previous (chained) signature value
   * @example `console.log(proof.signature) // '2b8504698e...'`
   */

  @Expose({ name: 'prevSignatureValue' })
  get prevSignature() {
    return this._prevSignatureValue
  }

  /**
   * Set the hex encoded previous (chained) signature value
   * @example `proof.signature = '2b8504698e...'`
   */

  set prevSignature(signature: string) {
    this._prevSignatureValue = signature
  }
