...
export type IPublicKeySectionAttrs =
  | IPublicKeySectionAttrsV0
  | IPublicKeySectionAttrsV013
...
const customObject = <T extends JsonLdObject, V extends keyof JsonLdObject>(
  key: string,
) => (value: V): T => (({ [key]: value } as unknown) as T)

const Hex = (key: string) =>
  customObject<Hex, string>(PublicKeyRepresentationType.Hex)(key)
const Base58 = (key: string) =>
  customObject<Base58, string>(PublicKeyRepresentationType.Base58)(key)
const Base64 = (key: string) =>
  customObject<Base64, string>(PublicKeyRepresentationType.Base64)(key)
const Pem = (key: string) =>
  customObject<Pem, string>(PublicKeyRepresentationType.Pem)(key)
const Eth = (key: string) =>
  customObject<Eth, string>(PublicKeyRepresentationType.Eth)(key)

export namespace PublicKeyRepresentationType {
  export function toBufferEncoding(typ: PublicKeyRepresentationType): string {
    switch (typ) {
      ...
      case PublicKeyRepresentationType.Eth:
        return 'hex'
      default:
        throw new Error('invalid public key representation type: ' + typ)
    }
  }

  export function has(typeString: string): boolean {
    switch (typeString) {
      case PublicKeyRepresentationType.Hex:
      case PublicKeyRepresentationType.Base58:
      case PublicKeyRepresentationType.Base64:
  ...
  export function fromStr(typeString: string): PublicKeyRepresentationType {
    switch (typeString) {
      case PublicKeyRepresentationType.Hex:
      ...
      default:
        throw new Error('invalid public key representation type: ' + typeString)
    }
  }

  export function keyToRep(
    typ: PublicKeyRepresentationType,
  ): (material: string) => PublicKeyForm {
    switch (typ) {
      case PublicKeyRepresentationType.Hex:
        return Hex
      case PublicKeyRepresentationType.Base58:
