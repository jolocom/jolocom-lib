...
import { ChainedSignature, EcdsaLinkedDataSignature } from '../../linkedDataSignature'
...
export class SignedCredential implements IDigestable {
  ...
  // FIXME TODO refactor out initialization of empty signature
  ...
  /**
   * Get the first proof (signature) ({@link EcdsaLinkedDataSignature}) member of the instance
  ...
  /**
   * Get all the proofs ({@link EcdsaLinkedDataSignature}) of the instance
   * @example `console.log(signedCredential.proofs) // [EcdsaLinkedDataSignature { ... }, ...]`
   */
  get proofs(): ILinkedDataSignature[] {
    return this._proof
  }

  /**
   * TODO: maybe use the signature value itself as a key??
   *
   *
   * Get a map of all proof chains ({@link ChainedSignature}) of the instance
   * The keys are signingKeyIds of the signers of the last signature in each
   * chain that is extractable from the `proofs` list ({@link SignedCredential.proofs})
   */
  get proofChains(): ProofChainsMap {
    const proofChains: ProofChainsMap = {}
    this.proofs.forEach((sig) => {
      const keyId = sig.signer.keyId
      const prevSigVal = sig.prevSignature
      if (prevSigVal) { // This signature is chained to a previous one
        const prevSig = this.proofs.find((sig) => sig.signature === prevSigVal)
        const prevChain = proofChains[prevSig.signer.keyId]

        if (!prevSig || !prevChain) { // something is wrong
          // maybe the proofs list is not ordered correctly?
          // maybe the referenced signature doesn't exist?
          throw new Error("TODO handle inconsistency")
        }

        proofChains[keyId] = prevChain.concat(sig)
      } else { // Not a chained sig
  ...
  async addChainedSignature(sig: ILinkedDataSignature, prevSigKeyId: string) {
    if (!prevSigKeyId || !this.proofChains[prevSigKeyId]) { // something is wrong
      throw new Error("TODO handle inconsistency")
    }
