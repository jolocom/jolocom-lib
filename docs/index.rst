...
* `Decentralized Identifiers (DIDs) <https://w3c-ccg.github.io/did-spec/>`_ - every entity making use of the Jolocom stack needs a DID if it wishes to issue / receive Verifiable Credentials, or initiate / participate in interactions with other Jolocom identities. The DID specification allows each agent in the network to generate a decentralized identifier and register / anchor it (the implementation of this opperation is normally defined in the corresponding DID Method specification). Once an identity has been "anchored", the corresponding DID will become resolvable by other network participants (TODO - add note about local first DID methods), and can be used as part of various supported interactions (e.g. for issuing and receiving Verifiable Credentials).
...
To recap, the Jolocom protocol can be used to:

* Create self-sovereign identities (e.g. for humans, organisations, smart agents).
