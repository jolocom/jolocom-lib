...
import { defaultTestDidDoc } from '../../utils/testVectors/didDocumentTestData'
...
describe('Jolo Did Method, bip39 recovery', async () => {
  ...
  didMethod.resolver.resolve = sinon.stub().resolves(
    Identity.fromDidDocument({
      //@ts-ignore Context typing issues
      didDocument: DidDocument.fromJSON(defaultTestDidDoc),
    }),
  )
  ...
  it('Should correctly derive keys / incept an identity given a seed', async () => {
    ...
    const {
      identityWallet,
      succesfullyResolved,
    } = await didMethod.recoverFromSeed(testSeed, 'pass')
