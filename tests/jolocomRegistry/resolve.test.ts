...
import { mockDid, didDocumentJSON, mockIpfsHash } from '../data/didDocument.data'
...
describe('Jolocom Registry - resolve', () => {
  ...
  it('should resolve with public profile', async () => {
    const extendedDidDoc = { ...didDocumentJSON, service: [mockPubProfServiceEndpointJSON] }
  ...
  it('should implement fetchPublicProfile', async () => {
    registry.ipfsConnector.catJSON = sinon.stub().resolves(publicProfileCredJSON)
    ...
    expect(registry.ipfsConnector.catJSON.getCall(0).args).to.deep.eq([mockIpfsHash])
  })
})
