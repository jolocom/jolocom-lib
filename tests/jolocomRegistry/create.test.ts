...
import { testPrivateIdentityKey, testPublicIdentityKey, testSeed } from '../data/keys.data'
...
describe('Jolocom Registry - create', () => {
  ...
  before(async () => {
    ...
    sandbox.stub(DidDocument, 'fromPublicKey').returns(DidDocument.fromJSON(didDocumentJSON))
  ...
  it('should create new identity', async () => {
    ...
    sandbox.assert.calledWith(Identity.fromDidDocument, { didDocument: DidDocument.fromJSON(expectedDidDoc) })
